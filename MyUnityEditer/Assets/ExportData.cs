...
/// <summary>
/// 親オブジェクトの「子供オブジェクト」すべての Transform（位置・回転・スケール）をCSVで出力する
/// </summary>
public class ExportData : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        string path = Application.dataPath + "/ChildrenTransformData.csv";
        File.WriteAllLines(path, lines);//File.WriteAllLines(ファイルのパス, 文字列の配列またはリスト);

        // 結果をUnityのConsoleに表示
        Debug.Log($"子オブジェクトのTransform情報を出力しました: {path}");
    }
}
