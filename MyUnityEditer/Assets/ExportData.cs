...
public class ExportData : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        lines.Add("Name,PosX,PosY,PosZ,RotX,RotY,RotZ,ScaleX,ScaleY,ScaleZ," +
            "GroupTag,HP,Armor" +",AttackPower," +
            "Speed,SearchPlaceRang,ViewingAngle,SearchDistance,ModelRotateSpeed");
        ...
        foreach (Transform child in m_parentObject.transform)//すべて入れていく
        {
            ...
            if (child.GetComponent<StatusData>())
            {
                ...
                viewingAngle = status.viewingAngle;
                searchDistance = status.searchDistance;
                modelRotateSpeed = status.modelRotateSpeed;
            }
        ...
        //　"," ごとに区切っていく
        // 名前と各データを1行にまとめて追加
        string line = $"{child.name},{pos.x},{pos.y},{pos.z}," +
                $"{rot.x},{rot.y},{rot.z},{scale.x},{scale.y},{scale.z}," +
                $"{tagName},{hp},{armor},{attackPower},{speed}," +
                $"{searchPlaceRang},{ viewingAngle},{searchDistance},{modelRotateSpeed}";
            lines.Add(line);//行を追加
        }
